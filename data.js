  ...
  weeks: [
    {
      ...
      selfPaced: [
        ...
        },
        {
          ...
          points: [
            "Public training-ai-fde repo with README + week folders; URL sent.",
    ...
    },
    {
      ...
      selfPaced: [
        {
          ...
          points: [
            ...
            "Start here: in the training repo, create a branch, make two commits, then manufacture a merge conflict against another branch and resolve it from the command line.",
      ...
      deliverable: "Brief: take a real internal training repo from clone to a clean, merged pull request, with one bug fixed by hand and one fixed with Claude Code.\n\nRequirements: (1) clone the internal training repo and get it running locally; (2) find a first small, genuine bug and fix it by hand, no agent, noting what the stack trace or error told you; (3) find a second genuine issue and fix it with Claude Code, reviewing the diff before accepting; (4) add at least one test that covers your change; (5) open one clean pull request — a clear title, a description of the change, and the tests; (6) get the PR through review and merged, with CI passing.\n\nSuggested steps: clone, install, run the test suite, and confirm it is green before you touch anything; orient using the Topic B codebase-reading method and write the three-question answer; reproduce the first bug, read the trace, fix it by hand, add or update a test; pick the second issue, brief Claude Code, review and accept its diff, confirm tests pass; branch, commit with clean messages, push, open the PR, respond to review, merge.\n\nDeliverables: the merged pull request link, plus a short written note (half a page) covering how you navigated the codebase, what the by-hand bug taught you, and where Claude Code helped or got in the way.\n\nStretch (optional): add a second test that would have caught the bug before it shipped; leave a constructive review comment on a peer's PR.",
      assessment: "Submit the merged PR link and the half-page note, then give a 3-minute walkthrough of your change: what was wrong, how you fixed it, and one thing the by-hand bug taught you about reading code. Judged on technical execution (does the fix work, are the tests meaningful, is the PR clean, did CI pass), handling ambiguity and failure (navigating an unfamiliar repo and a real bug without freezing), communication (can a non-author follow your PR and walkthrough), and ownership (did you drive it to merged). Done means a reviewer who has never seen your change can read the PR, understand what it does and why, see the test that proves it, and watch CI pass — all without asking you a question.",
    },
    {
      num: 2,
      phaseId: "p1",
